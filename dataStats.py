...
def DiffMeansNullHypTest(a, b, iters=1000, onesided=False):
    """Generates a list of differences in means of two data sets from randomized simulations of pooled data.
       Results are values that can be generated from cases in which the null hypothesis is true. 
       Can then use the 'max' built-in to find what the most extreme value is and compare to statistic from original data.
...
def OneSampleZtestForProportion(p, p_pop, n):
    ...
    The test measures whether an observed proportion differs significantly from a population proportion.
    ...
    """
    z = (p - p_pop) / np.sqrt(p_pop * (1 - p_pop) / n)
